...
for state in r.json():
    ...
    # Dumping models
    joblib.dump(model_gpr, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-gpr-0.1.pkl'.format(state['uf'], TIMESTAMP)))
    joblib.dump(model_metadata_gpr, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-gpr-metadata-0.1.pkl'.format(state['uf'], TIMESTAMP)))

    joblib.dump(model_full_linreg, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-linear-polynomial-0.1.pkl'.format(state['uf'], TIMESTAMP)))
    joblib.dump(model_metadata_linreg, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-linear-polynomial-metadata-0.1.pkl'.format(state['uf'], TIMESTAMP)))

    joblib.dump(model_linreg_simple, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-linear-regression-0.1.pkl'.format(state['uf'], TIMESTAMP)))
    joblib.dump(model_metadata_linreg_simple, os.path.join(MODELS_FOLDER, state['uf'], 'model-{}-{}-linear-regression-metadata-0.1.pkl'.format(state['uf'], TIMESTAMP)))
